...
## The problem
Relationship managers at a bank each own a book of customers and need answers to business questions — who's about to churn, who's high-value, who should get a loan, what should we offer them — but getting those answers normally requires either digging through raw data yourself or waiting on a data analyst to write a query. On top of that, whoever answers those questions needs to only see their own customers, not everyone's, and any AI involved can't be allowed to invent numbers about real people in a banking context. So really it was three problems stacked together: speed (no technical bottleneck between a question and an answer), access control (RMO sees only their book, admin sees everyone's), and trust (nothing gets hallucinated, everything is traceable).

So ML Hub is a banking-focused AI assistant platform designed to help customer relationship teams, relationship managers, and business users query customer intelligence in natural language.
...
1. Data first. A SQL Server warehouse with a proper star schema — dimensions (customers, branches, RMOs, products) and facts (accounts, churn, CLV, fraud, lending, recommendations, engagement) — seeded with 100,000 synthetic customers. Nothing downstream could exist without this being right first.

2. A backend that only knows how to do safe things. Not a general query layer — four fixed, parameterized, read-only endpoints. This was the first guardrail, and it's structural rather than a rule someone has to remember: there's no code path for a random query to reach the database at all.

3. An AI layer that picks tools instead of writing SQL. This was the single most important architectural call in the whole project. Claude never generates a query — it only ever selects from a small menu of pre-approved functions. That one decision is what let everything after it be trustworthy: the model literally cannot do the dangerous thing, so you don't have to hope it behaves.

4. Governance made visible, not just logged. Every question, every tool call, every answer written to an audit trail — and then surfaced in the UI itself as an expandable "how this was answered" panel, so the traceability isn't buried in a file nobody reads.

5. A real chat interface. Rebuilt from a sectioned marketing-style page into an actual ChatGPT-shaped experience — streaming answers, markdown rendering, conversation memory so follow-up questions ("is that customer eligible for lending?") resolve correctly against prior turns.

6. Access control enforced where it can't be argued with. RMO login scoped every single query server-side by PrimaryRmoId — not a UI restriction, a backend one. Then an admin tier on top: password-gated (since full-bank access is a much bigger privilege than an RMO's own book), able to browse any RMO's book or go bank-wide, with that scope injected automatically into every tool call rather than left to the AI's discretion.

7. Finally, real machine learning underneath the model outputs. Everything up to this point still had scores generated by hand-written formulas. The last phase replaced that for a second cohort: trained actual regressors and classifiers on the original 100,000 customers as ground truth, generated 100,000 brand-new customers with more realistic (not perfectly even) demographics, and predicted every one of churn, CLV, fraud, lending eligibility, default risk, and product recommendations onto them — writing the results straight into the same warehouse tables, so the new customers are indistinguishable in structure from the original ones.
